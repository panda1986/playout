"use strict";
bravo_bls.controller("vodCtrl", ["$scope", "$location", "bls_check", "bls_api", "$uibModal", "video_upload", "$filter", "growl", "$rootScope", "$timeout", "third_menu",
    function($scope, $location, bls_check, bls_api, $uibModal, video_upload, $filter, growl, $rootScope, $timeout, third_menu) {
        ...
        function get_video_list() {
            ...
            bls_api.get_video_list($scope.video_data, function(res) {
                ...
                bls_check.init_check($scope, $scope.init_check_data, 'video_id');
        ...
        $scope.modify = function(video_id, name) {
            ...
            var modalInstance = $uibModal.open({
                animation: true,
                templateUrl: 'views/partials/vod_modify.html',
        ...
        $scope.checked = function (check) {
            if (check) {
                ...
                async_refresh2.request(bls_refresh.interval);
            }
        }
        ...
        $scope.delete_video = function(video_id) {
            ...
            bls_check.batch_operation(obj, function(data) {
                ...
                modalInstance.result.then(function () {
                    bls_api.delete_video(data, function(res) {
                        growl.addWarnMessage("删除视频成功", {ttl: bls_prompt.success});
                        get_video_list();
        ...
        $scope.delete_download_videos = function(download_id) {
            ...
            modalInstance.result.then(function() {
                bls_api.delete_download_videos({"download_ids": [download_id]}, function(data) {
                    growl.addWarnMessage("删除视频成功", {ttl: bls_prompt.success});
                    get_video_list();
                });
            });
        ...
        $scope.check_details = function(index, video_id) {
            ...
            switch (title) {
                case '技术审核':
                    modal_info.templateUrl = 'views/partials/tec_check.html';
                    modal_info.controller = 'tecCheckCtrl';
                    modal_info.size = 'w800';
                    break;
                case '内容审核':
                    modal_info.templateUrl = 'views/partials/content_check.html';
                    modal_info.controller = 'contentCheckCtrl';
                    modal_info.size = 'w800';
                    break;
                case '转码':
                    modal_info.templateUrl = 'views/partials/transcode_check.html';
        ...
        $scope.select_upload = function(option) {
            var modalInstance = $uibModal.open({
                animation: true,
                templateUrl: 'views/partials/upload_material.html',
        ...
        $scope.import_material = function(options) {
            var modalInstance = $uibModal.open({
                animation: true,
                templateUrl: 'views/partials/import_material.html',
        ...
        $scope.import_videos = function(options) {
            var modalInstance = $uibModal.open({
                animation: true,
                templateUrl: 'views/partials/import_material_cdv.html',
        ...
        async_refresh2.refresh_change(function() {
            bls_refresh.is_spinner = false;
            get_video_list();
            async_refresh2.request();
        }, bls_refresh.interval);
        async_refresh2.request(bls_refresh.interval);
    }
]);
bravo_bls.controller("tecCheckCtrl", ["$scope", "$uibModalInstance", "bls_api", "info", "growl",
    function($scope, $uibModalInstance, bls_api, info, growl) {
        bls_api.get_review_detail({video_id: info.video_id, review_type: 0}, function(res) {
        ...
        $scope.tec_pass = function() {
            ...
            bls_api.video_review(data, function(res) {
                growl.addSuccessMessage("审核通过", {ttl: bls_prompt.success});
                Bflag = true;
...
bravo_bls.controller("contentCheckCtrl", ["$scope", "$uibModalInstance", "bls_api", "info", "growl",
    function($scope, $uibModalInstance, bls_api, info, growl) {
        bls_api.get_review_detail({video_id: info.video_id, review_type: 1}, function(res) {
        ...
        $scope.content_submit = function() {
            ...
            bls_api.submit_content_review(data, function(res) {
...
bravo_bls.controller("transcodeCheckCtrl", ["$scope", "$uibModalInstance", "bls_api", "info", "growl",
    function($scope, $uibModalInstance, bls_api, info, growl) {
        bls_api.get_video_with_coding_info({video_id: info.video_id}, function(res) {
            $scope.transcode_check = res.result;
        });

        $scope.redo_transcoding = function(task_id) {
            bls_api.redo_transcoding({video_id: info.video_id, task_id: task_id}, function(res) {
                growl.addWarnMessage("开始重新转码", {ttl: bls_prompt.success});
                $uibModalInstance.close();
            });
        };
...
bravo_bls.controller("uploadMaterialCtrl", ["$scope", "$rootScope", "$uibModalInstance", "info", "bls_api", "growl",
    function($scope, $rootScope, $uibModalInstance, info, bls_api, growl) {
...
// 导入素材 (从VMS导入素材)
bravo_bls.controller('importMaterialCtrl', ['$scope', '$uibModalInstance', 'bls_api', 'growl', 'info',
    function($scope, $uibModalInstance, bls_api, growl, info) {
        $scope.bigCurrentPage = 1;
        ...
        bls_api.get_vms_directories({}, function(data) {
        ...
        function get_video_list() {
            ...
            bls_api.get_vms_product_videos($scope.api_data, function(data) {
        ...
        $scope.ok = function () {
            if (!send_videos.length) return growl.addWarnMessage('请选择素材', {ttl: bls_prompt.error});
            if (typeof $scope.type !== 'number') return growl.addWarnMessage('请选择要导入的分类', {ttl: bls_prompt.error});
            ...
            for (var i = 0; i < send_videos.length; i++) {
                ...
                if (data.fileType === 'm3u8') return growl.addWarnMessage('m3u8类型素材不可导入', {ttl: bls_prompt.error});
            }

            if (temp_arr.length !== _.uniq(temp_arr).length) return growl.addErrorMessage('不可选择同名的素材', {ttl: bls_prompt.error});
            ...
            //    if (!res.result.length) {
                    ...
                    bls_api.post_video_info(send_info, function(data) {
                        if (data.status == 'success') {
                            growl.addSuccessMessage('素材已加入下载列表中', {ttl: bls_prompt.success});
                            $uibModalInstance.close();
                        } else {
                            //var same_arr = _.intersection(data.result, temp_arr_format);
                            //growl.addErrorMessage(same_arr.join('，') + '重复导入', {ttl: bls_prompt.error});
                            growl.addErrorMessage(data.result, {ttl: bls_prompt.error});
                        }
                    });
              //  } else {
              //      growl.addWarnMessage(res.result.join('，') + '文件名重复', {ttl: bls_prompt.error});
              //  }
            //});
        };
...
// 导入素材（从新奥特导入素材）
bravo_bls.controller("importVideosCtrl", ["$scope", "$uibModalInstance", "bls_api", "growl", "info", "$sce",
    function($scope, $uibModalInstance, bls_api, growl, info, $sce) {
        $scope.bigCurrentPage = 1;
        ...
        function get_video_list() {
            ...
            bls_api.get_cdv_product_videos($scope.api_data, function(data) {
                $scope.materials = data.result.videos;
                angular.forEach($scope.materials, function(data) {
        ...
        $scope.ok = function () {
            if (!send_videos.length) {
                return growl.addWarnMessage("请选择素材", {ttl: bls_prompt.error});
            }
            if ((typeof $scope.type).toLowerCase() != "number") {
                return growl.addWarnMessage("请选择要导入的分类", {ttl: bls_prompt.error});
            }
            ...
            if (m3u8_exist) return growl.addWarnMessage('不支持导入m3u8类型素材', {ttl: bls_prompt.error});
...
bravo_bls.controller("vodModifyCtrl", ["$scope", "$uibModalInstance", "info", "bls_api", "growl",
    function($scope, $uibModalInstance, info, bls_api, growl) {
        ...
        $scope.ok = function() {
            ...
            bls_api.update_video(send_info, function(data) {
